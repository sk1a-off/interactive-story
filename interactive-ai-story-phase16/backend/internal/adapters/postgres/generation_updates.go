...
func (s *GenerationUpdates) Append(ctx context.Context, u app.Update) (app.Update, error) {
 ...
 SELECT $1,seq,$2,$3,$4 FROM next
 RETURNING sequence`, u.GenerationID, string(u.Phase), u.TextDelta, u.Error).Scan(&u.Sequence)
	return u, e
}
func (s *GenerationUpdates) List(ctx context.Context, g id.ID, after int64) ([]app.Update, error) {
	rows, e := s.pool.Query(ctx, `SELECT sequence,phase,text_delta,error_text FROM generation_updates WHERE generation_job_id=$1 AND sequence>$2 ORDER BY sequence`, g, after)
	if e != nil {
		return nil, e
	}
	defer rows.Close()
	...
	for rows.Next() {
		var u app.Update
		u.GenerationID = g
		if e = rows.Scan(&u.Sequence, &u.Phase, &u.TextDelta, &u.Error); e != nil {
			return nil, e
		}
		out = append(out, u)
	}
